...
// ======================== [新增] 极速脉冲计数核函数 ========================
__global__ void k_count_spikes(const float* spikes, unsigned long long* total_spikes, int size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        // 如果发放时间小于 INF_TIME，说明该神经元发放了脉冲
        if (spikes[idx] < INF_TIME - 1.0f) {
            atomicAdd(total_spikes, 1ULL);
        }
    }
}

// 封装计数调用
void count_layer_spikes(float* d_spikes, int size, unsigned long long* d_total_spikes) {
    k_count_spikes<<< (size + 255) / 256, 256 >>>(d_spikes, d_total_spikes, size);
...
int main(int argc, char* argv[]) {
    ...
    // [新增] 申请显存用于统计所有隐藏层发出的总脉冲数
    unsigned long long* d_total_spikes;
    CHECK_CUDA(cudaMalloc(&d_total_spikes, sizeof(unsigned long long)));
    CHECK_CUDA(cudaMemset(d_total_spikes, 0, sizeof(unsigned long long)));

    // 计算 VGG16 隐藏层神经元总数 (Conv1 到 FC2)
    // 65536*2 + 32768*2 + 16384*3 + 8192*3 + 2048*3 + 512*2 = 277504
    double total_hidden_neurons_per_image = 277504.0;
    ...
    std::cout << ">>> Start processing 10000 images for Sparsity Measurement...\n";
    ...
    for (int img_idx = 0; img_idx < 10000; ++img_idx) {
        ...
        count_layer_spikes(d_c2, 16*16*128, d_total_spikes);

        launch_conv(d_c2,   d_c2_1, 16, 16, 128, 16, 16, 128, 3, layers[2].tc_fire, layers[2].td, d_event_buffer, layers);
        count_layer_spikes(d_c2_1, 16*16*128, d_total_spikes);
    ...
    printf("\n\n========================================================\n");
    printf(">>> 10000 Images Inference Finished.\n");
    printf(">>> Final Accuracy: %.2f%%\n", (float)correct_count / 10000.0f * 100.0f);

    // ======================== 结算稀疏度 ========================
    unsigned long long h_total_spikes = 0;
    CHECK_CUDA(cudaMemcpy(&h_total_spikes, d_total_spikes, sizeof(unsigned long long), cudaMemcpyDeviceToHost));

    double avg_spikes_per_image = (double)h_total_spikes / 10000.0;
    double spike_sparsity = (avg_spikes_per_image / total_hidden_neurons_per_image) * 100.0;

    printf("--------------------------------------------------------\n");
    printf("Total Spikes (10k images)   : %llu\n", h_total_spikes);
    printf("Avg Spikes per Image        : %.2f\n", avg_spikes_per_image);
    printf("Hidden Neurons per Image    : %.0f\n", total_hidden_neurons_per_image);
    printf(">>> Overall Spike Sparsity  : %.4f %%\n", spike_sparsity);
    printf("========================================================\n\n");
