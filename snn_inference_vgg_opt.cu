...
int main() {
    ...
    std::cout << ">>> Loading " << B << " CIFAR-10 Images..." << std::endl;
    std::vector<float> all_imgs(B * 3072);
    for (int i = 0; i < B; ++i) {
    ...
    CHECK_CUDA(cudaMalloc(&d_img, B * 3072 * 4)); CHECK_CUDA(cudaMemcpy(d_img, all_imgs.data(), B * 3072 * 4, cudaMemcpyHostToDevice));
    ...
    std::cout << ">>> Starting SNN Fast Matrix/Warp-Cooperative Batch Inference..." << std::endl;
    ...
    float accuracy = static_cast<float>(correct) / B * 100.0f;
    std::cout << "=======================================" << std::endl;
    std::cout << "Batch Inference Completed on " << B << " Images" << std::endl;
    std::cout << "Optimized Accuracy: " << accuracy << "%" << std::endl;
    std::cout << "=======================================" << std::endl;
